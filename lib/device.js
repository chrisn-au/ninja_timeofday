...
function processTimes()     
{
      ...
      // retrieve the time from the SunCalc object and add the delta (+/-)
       ...
       phases.forEach(function (lc) {
         ...
         if (lc.alertTime > 0) {
            app.log.info(location.name +  ': setting up ' + lc.display + ' at ' + times[lc.name])
            setTimeout(updateNinja, lc.alertTime , lc.display)
         } else {
            app.log.warn(location.name +  ': not setting up ' + lc.display + ' as ' + times[lc.name] + ' is in the past')
         }
       });
       ...
       app.log.info(location.name + ': scheduling recalc for tomorrow: ' + tomorrow.addSeconds(50))
