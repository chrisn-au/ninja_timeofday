...
function Device(location) {
  ...
  function setdayTimes()
 {
    ...
    var phase = buildLoc(location);

    processTimes(phase);
    updateNinja(phase.state);
 }
...
function processTimes(phases)     
{
      ...
      // retrieve the time from the SunCalc object and add the delta (+/-)
       ...
       setTimeout(updateNinja, (setdayTimes, (n.getSecondsBetween(tomorrow) * 1000) + 50000));         
